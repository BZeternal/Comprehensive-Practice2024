    ...
    <div style="display: flex; height: 100%;">
        ...
        <el-card style="flex: 2.2; height: 100%;">
           <div style="display: flex; flex-direction: column; margin: 2% 0 0 8%; height: 33.5rem;">
                <h2>{{ problem.id }}. {{ problem.question }} <el-icon v-if="problem.right=='success'" color="green"><SuccessFilled /></el-icon> <el-icon v-if="problem.right==='danger'" color="red"><CircleCloseFilled /></el-icon></h2>
                <el-button @click="open" style="font-size: 1.05rem;margin: 3px 1%; width: 120px; height: 35px;" key="primary" type="primary" text bg v-if="problem.right==='danger'">查看题解</el-button>
                <el-checkbox-group :disabled="problem.disable" style="display: flex; flex-direction: column;" v-model="problem.answers" size="large">
                ...
                <el-image v-if="problem.url != ''" style="margin-top:2% ;margin-left: 20%; width: 200px; height: 200px" :src="problem.url" fit="fill" />      
           </div>
           <el-button v-if="rework" style="font-size: 1.05rem;margin-left: 1%; width: 120px; height: 35px;" key="primary" type="primary" text bg @click="redo">再做一次</el-button>
            <el-button style="font-size: 1.05rem;margin-left: 70%; width: 120px; height: 35px;" key="primary" type="primary" text bg @click="ok">确定</el-button>
        </el-card>
    </div>
...
const refresh_practice = () => {
    practiceApi.getQuestion(1,'c1').then(resp=>{

<template>
  ...
  >
    ...
    <el-card style="flex: 2.2; height: 100%">
      ...
      >
        ...
        <el-button
          @click="open"
          style="font-size: 1.05rem; margin: 3px 1%; width: 120px; height: 35px"
          key="primary"
          type="primary"
          text
          bg
        ...
        >
        <el-checkbox-group
          :disabled="problem.disable"
          style="display: flex; flex-direction: column"
          v-model="problem.answers"
          size="large"
        >
        ...
        <el-image
          v-if="problem.url != ''"
          style="margin-top: 2%; margin-left: 20%; width: 450px; height: 280px"
          :src="problem.url"
          fit="fill"
        />
      </div>
      <el-button
        v-if="rework"
        style="font-size: 1.05rem; margin-left: 1%; width: 120px; height: 35px"
        key="primary"
        type="primary"
        text
        bg
      ...
      >
      <el-button
        style="font-size: 1.05rem; margin-left: 70%; width: 120px; height: 35px"
        key="primary"
        type="primary"
        text
        bg
  ...
  >
    <el-select
      ...
      placeholder="选择科目"
      size="large"
      style="width: 350px; margin-top: 5%"
    >
      <el-option key="1" label="科目1" value="1"></el-option>
      <el-option key="4" label="科目4" value="4"></el-option>
    </el-select>
    <el-button
      @click="start_practice"
      style="width: 12rem; height: 12rem; margin-top: 2rem; font-size: 1.5rem"
      type="primary"
      circle
      ><el-icon size="2.6rem"> <Edit /> </el-icon>开始刷题</el-button
    >
  </div>
</template>
...
const refresh_practice = () => {
  loading.value = true;
  practiceApi
    .getQuestion(subject.value, userSore.aCarType)
    .then((resp) => {
      if (resp.data.reason == "ok") {
        ...
        for (let i = 0; i < resp.data.result.length; i++) {
          ...
          if (problems[problems.length - 1].item1 == "")
            problems[problems.length - 1].item1 = "对";
          if (problems[problems.length - 1].item2 == "")
            problems[problems.length - 1].item2 = "错";
          flags.push({
            id: i + 1,
            type: "info",
            answers: [],
...
const open = () => {
  ElMessageBox.alert(
    problem.explains + "   正确答案：" + problem.answer,
    "题目解析",
    {
